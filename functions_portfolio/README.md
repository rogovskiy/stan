...
1. **`portfolio_weekly_publish`** (scheduled) — reads all portfolio document IDs from Firestore and **publishes one Pub/Sub message per portfolio** to `portfolio-channel-exposure-requests`.
...
**Topic:** `portfolio-channel-exposure-requests`
...
The **`portfolio_weekly_publish`** runtime service account needs **`roles/pubsub.publisher`** on the topic (or project-level Pub/Sub Publisher). In GCP Console: Pub/Sub → topic → Permissions, or:
...
```bash
firebase deploy --only functions:portfolio:portfolio_weekly_publish,functions:portfolio:portfolio_channel_exposure_refresh
```
