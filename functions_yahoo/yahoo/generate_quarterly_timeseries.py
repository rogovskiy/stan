...
CLI: run with ``python -m yahoo.generate_quarterly_timeseries`` from functions_yahoo
(with venv, ``make vendor``, and Firebase env — see run_local / .env.local).
"""
...
def validate_firebase_config() -> None:
    """Validate required Firebase environment variables (CLI only)."""
    import os
    ...
    if missing_vars:
        ...
        for var in missing_vars:
            print(f'   - {var}')
        print('\n💡 For local CLI: load ../data-fetcher/.env.local (see run_local.py).')
        sys.exit(1)
...
class QuarterlyTimeSeriesGenerator:
    ...
    def _extract_timeseries_data(self, quarterly_data: List[Dict[str, Any]],
        ...
        timeseries_data = []
        quarters_with_reasons = []

        for quarter in quarterly_data:
            try:
                data_point = {
                    'date': quarter.get('period_end_date'),
    ...
    def get_cached_quarterly_timeseries(self, ticker: str, max_age_hours: int = 24) -> Optional[Dict[str, Any]]:
        return self.timeseries_service.get_quarterly_timeseries(ticker, max_age_hours)
...
def main() -> None:
    """CLI: load dotenv from data-fetcher .env.local when present."""
    import os
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='''
Examples:
  python -m yahoo.generate_quarterly_timeseries AAPL
  python -m yahoo.generate_quarterly_timeseries AAPL --verbose
  (from functions_yahoo after: source venv/bin/activate && make vendor)
        '''
    )
