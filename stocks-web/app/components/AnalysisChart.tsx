...
export default function AnalysisChart({
  ...
  const chartData = useMemo(() => {
    ...
    if (chartSpec.frequency === 'annual') {
      ...
      // Quarterly frequency - use data as-is
  ...
  const allValues = chartData.flatMap(d => 
    chartSpec.metrics.map(metric => d[metric] as number).filter(v => typeof v === 'number')
  );
        ...
        <BarChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 60 }}>
          ...
          {chartSpec.metrics.map((metric, index) => (
            <Bar
              ...
              dataKey={metric}
              stackId="a"
              fill={METRIC_COLORS[metric] || METRIC_COLORS.default}
              name={metric}
            />
          ))}
        </BarChart>
