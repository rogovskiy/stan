  ...
  return (
    <div className={'bg-gradient-to-br ' + (isRisk ? 'from-red-50 to-orange-50' : 'from-white to-gray-50') + ' rounded-lg border ' + (isRisk ? 'border-red-200' : 'border-gray-200') + ' p-2 hover:shadow-sm transition-shadow'}>
      ...
      <div className="flex items-center justify-between mb-2">
        <h4 className={'text-sm font-bold ' + (isRisk ? 'text-red-900' : 'text-gray-900') + ' leading-tight'}>{driver.factor || 'Unknown Factor'}</h4>
        {driver.contribution_percent !== undefined && (
          <span className={'text-xs font-semibold ' + (isRisk ? 'text-red-700 bg-red-100' : 'text-blue-700 bg-blue-100') + ' px-1.5 py-0.5 rounded'}>
      ...
      {driver.thesis_points && driver.thesis_points.length > 0 && (
        <div className="mb-2 space-y-1.5">
          {driver.thesis_points.slice(0, 2).map((point, ptIdx) => (
          ...
          {driver.thesis_points.length > 2 && (
            <div className="text-xs text-gray-500 mt-1">
              +{driver.thesis_points.length - 2} more
            </div>
          )}
        </div>
      )}
